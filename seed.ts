...
async function seed() {
  ...
  for (const test of mockTests) {
    await db.collection('mockTests').doc(test.id).set(test);
  ...
  for (const q of physicsQuestions) {
    await db.collection('mockTests').doc(q.testId).collection('questions').doc(q.id).set(q);
  ...
  for (const mat of materials) {
    await db.collection('studyMaterials').doc(mat.id).set(mat);
